...
`H3 条件与 Latent 切换` 在选定分支后处理音频，默认 `audio_mode=native` 保持原 Latent。选择 `lock_source`（锁定）或 `remix_source`（重混）时，连接 `drive_audio` 和 H3 `audio_vae`；重混使用 `audio_denoise_strength` 控制音频噪声遮罩，锁定时遮罩为 0。源音频编码后按目标长度截断或补零，保留原视频 Latent 和视频遮罩。

新增输出 `mux_audio` 接视频合成节点的音频输入：优先输出 `final_audio`，否则输出 `drive_audio`，两者均未连接时为空。它是指定的合成音轨，不是采样后生成的声音；若要使用生成／重混后的声音，应连接采样后音频解码节点的输出。旧工作流默认使用原生模式，原有前两个输出口顺序保持不变。
...
- **Outputs 参考图尺寸**：`match` 按输出宽 × 高的面积处理；`1.2`～`3.1`（步进 0.1）按输出面积 × 倍率处理。保留比例、只缩小，尺寸取整到 32 的倍数；`max`（默认）原图透传，由官方节点执行原生尺寸上限处理。
- Outputs 的宽、高和帧数统一沿用素材板设置，参考图面积倍率也按素材板的输出宽高计算。将 `width / height / frames` 输出分别接官方节点的 `width / height / length`，图片接参考图输入，官方“参考图像尺寸”设为 `max`，避免再次按输出面积缩小。视频、视频音频和独立音频沿用原有输出处理。
...
- 内置变量 `mediaBorad`：在“获取 H3mb 内置变量”节点中选择，即可获取同一工作流中已启用的“H3 生视频模式控制”的 `media_board` 输出，无需额外 Set 节点。多个模式控制节点时首次选择左侧最近的节点并记住绑定。
